...
class ConnectState extends State<Connect> {
  ...
  Future<void> checkPrefs() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? deviceGlobalCode = prefs.getString('globalConnectCode');
    String? userGlobalCode;
    FirebaseFirestore.instance
        .collection('users')
        .doc(user!.uid)
        .get()
        .then((DocumentSnapshot documentSnapshot) {
      if (documentSnapshot.exists) {
        userGlobalCode = documentSnapshot['GLOBALCode'];
      }
    });
    if(userGlobalCode != null && deviceGlobalCode != userGlobalCode){
      await prefs.setString('globalConnectCode', userGlobalCode!);
    }
    return;
  }
                ...
                children: <Widget>[
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      ElevatedButton(
                        onPressed: accessCode != null ? null : _generateAccessCode ,
